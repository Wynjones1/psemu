...
INSTANTIATE_TEST_CASE_P(
	ADDI, ImmediateInstruction, ::testing::Values(
		...
		PARAM(OpcodeEncoding::ADDI, 1, 1, 2, false),
		PARAM(OpcodeEncoding::ADDI, 0, UINT16_MAX, UINT16_MAX, false),
		PARAM(OpcodeEncoding::ADDI, 1, UINT16_MAX, uint32_t(UINT16_MAX) + 1, false)
));
...
INSTANTIATE_TEST_CASE_P(
	ADDIU, ImmediateInstruction, ::testing::Values(
		...
		PARAM(OpcodeEncoding::ADDIU, 1, 1, 2, false),
		PARAM(OpcodeEncoding::ADDIU, 0, UINT16_MAX, UINT16_MAX, false),
		PARAM(OpcodeEncoding::ADDIU, 1, UINT16_MAX, uint32_t(UINT16_MAX) + 1, false),
		PARAM(OpcodeEncoding::ADDIU, UINT32_MAX, UINT16_MAX, UINT16_MAX - 1, false)
));
